"""
Truly optimized kernel with merged time gradient calculations.
"""
import numpy as np


class NumpyKernelMergedGradients:
    ...
    def _compute(self, params, data, norm=None):
        """
        Compute forward pass and gradients with merged time gradient calculations.
        """
        ...
        g_bw = np.dot(g, self.matrix_gamma)  # Cache this
        ...
        fa = np.dot(ka, self.matrix_angle)  # Cache this
        ...
        # Key optimization: Merge repeated calculations
        
        # Common factor for time derivatives
        time_half = time / 2
        time_quarter = time / 4
        
        # eL and eH derivatives - compute common parts once
        # deL_dGamma = -time/2 * eL = -time_half * eL
        # deL_dDeltaGamma = -time/4 * eL = -time_quarter * eL
        # deL_dDeltaM = -1j * time/2 * eL = -1j * time_half * eL
        # Similarly for eH
        
        # Compute gp and gm derivatives more efficiently
        # dgp_dX = (deL_dX + deH_dX) / 2
        # dgm_dX = (deL_dX - deH_dX) / 2
        
        # Pre-compute probability gradients
        ...
        # Merge common gradient factors - compute once
        # grad_common = dQ_dpb * d_pb_dgp + dQ_dpbbar * d_pbbar_dgp
        # grad_common_gm = dQ_dpb * d_pb_dgm + dQ_dpbbar * d_pbbar_dgm
        grad_common_gp = dQ_dpb * d_pb_dgp + dQ_dpbbar * d_pbbar_dgp
        grad_common_gm = dQ_dpb * d_pb_dgm + dQ_dpbbar * d_pbbar_dgm
        
        # Now compute all three gradients in one pass
        # For Gamma: dgp_dGamma = -time_half * (eL + eH)/2 = -time_half * gp
        #           dgm_dGamma = -time_half * (eL - eH)/2 = -time_half * gm
        dgp_dGamma = -time_half * gp
        dgm_dGamma = -time_half * gm
        
        # For Delta_Gamma: dgp_dDeltaGamma = -time_quarter * (eL + eH)/2 = -time_quarter * gp
        #                 dgm_dDeltaGamma = -time_quarter * (eL - eH)/2 + time_quarter * gm
        # Actually: dgp_dDeltaGamma = (-time_quarter * eL + time_quarter * eH) / 2
        #          = -time_quarter * (eL - eH) / 2 = -time_quarter * gm
        dgp_dDeltaGamma = -time_quarter * gm
        dgm_dDeltaGamma = -time_quarter * gp
        
        # For Delta_m: dgp_dDeltaM = (-1j * time_half * eL + 1j * time_half * eH) / 2
        #             = 1j * time_half * (eH - eL) / 2 = -1j * time_half * gm
        #             dgm_dDeltaM = (-1j * time_half * eL - 1j * time_half * eH) / 2
        #             = -1j * time_half * (eL + eH) / 2 = -1j * time_half * gp
        dgp_dDeltaM = -1j * time_half * gm
        dgm_dDeltaM = -1j * time_half * gp
        
        # Compute gradients using merged formula
        ...
        # For pbbar, merge the calculation
