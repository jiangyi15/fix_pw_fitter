#!/usr/bin/env python3
"""Test the merged-index kernel against the reference numpy kernel."""
import sys, os
...
for n in [N_EVENTS, 128, 256]:
    ...
    # Merged index
    test = MergedIndexKernel(kernel_config)
    Q_t, grads_t, P_t = test._compute(params, data)
    ...
    for key in ['ck', 'm0', 'g0', 'scalar']:
        ...
        rel_diff = np.max(np.abs(g_test - g_ref)) / (np.max(np.abs(g_ref)) + 1e-30)
        assert rel_diff < 1e-12, f"grad_{key} mismatch: rel={rel_diff:.2e}"
    
    print(f"  n={n:>4}: ✓ All gradients match (max rel < 1e-12)")

print("\n✓ Merged-index kernel verified!")
