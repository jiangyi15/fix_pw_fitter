"""
core.py  –  FpwFitter class  (CUDA CFFI wrapper).
"""
...
class FpwFitter:
    ...
    def from_M(cls, F_data, w_data, B_data, M, N_b, purity=1.0):
        ...
        err = lib.fpw_create(n_data, n_proj, n_comp,
                             p_Fd, p_wd, p_Bd, p_M, N_b, purity, handle)
        if err != 0:
    ...
    @classmethod
    def from_mc_file(cls, F_data, w_data, B_data, F_mc_file, w_mc, B_mc,
                     F_mc_shape, purity=1.0, chunk_size=100_000):
        """Create from F_mc stored as raw-binary memmap file."""
    ...
    def evaluate(self, c, return_P=False):
        ...
        err = self._lib.fpw_evaluate(self._handle, p_cr, p_ci, p_nll, p_gr, p_gi, p_P)
        if err != 0:
