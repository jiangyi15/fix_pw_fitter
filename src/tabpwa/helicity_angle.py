...
# ---------------------------------------------------------------------------
# tabpwa DecayChain adapters
# ---------------------------------------------------------------------------
# tabpwa's own decay model already is the tree: a chain's ``.decays`` list is
...
def _spin_of(particle):
    """Numeric spin of an tabpwa Particle (int/float/Fraction-like)."""
...
def decay_chain_leaves(chain):
    """Final-state tabpwa Particles of a chain, in pre-order."""
    decays = chain.decays
    by_core = {}
    for i, d in enumerate(decays):
        by_core.setdefault(d.core.name, []).append(i)
...
def chain_angular_table(chain, verbose=False):
    """Build the canonical angular table for an tabpwa DecayChain."""
    import itertools as _it
    from fractions import Fraction as _Fr
...
def angular_model(chains, verbose=False):
    ...
    Args:
        chains: iterable of tabpwa DecayChain objects.
...
def wave_terms_canonical(chain, ls):
    ...
    Given an tabpwa DecayChain and one per-vertex (l, s) combination, returns
