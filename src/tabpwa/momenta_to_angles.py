...
def build_node_momenta(chain, final_momenta):
    ...
    Args:
        chain: tabpwa DecayChain (DFS pre-order decays).
...
def _chain_su2_frames(chain, mom_name_arrays):
    ...
    """
    from tabpwa.helicity_angle import decay_chain_leaves
    from tabpwa.su2 import (Identity, _mul, Boost_z_from_p, Rotation_y,
                            Rotation_z)
    leaves = decay_chain_leaves(chain)
    names = [o.name for o in leaves]
    arrs = [np.asarray(mom_name_arrays[nm], dtype=float) for nm in names]
...
def _cm_reference_frames(mom_name_arrays, spinful_names):
    ...
    """
    from tabpwa.su2 import (Identity, _mul, Rotation_y, Rotation_z, Boost_z,
...
def aligned_euler_from_momenta(chains, mom_name_arrays, spinful_names,
    ...
    * ``align_ref="chain"``: rule1 - the reference is the chain where the
    ...
    Args:
        chains: list of tabpwa DecayChain (active topologies).
    ...
    if len(chains) <= 1:
        return None
    from tabpwa.su2 import Identity, _mul, inv as su2_inv, get_euler_angle
...
def momenta_to_data_angles(momenta, weight=None, frac=None, time=None):
    """B→4π data dict identical to ``tabpwa.momenta_to_data``.
    ...
    """
    from tabpwa.momenta_to_data import momenta_to_data as _orig
...
# ---------------------------------------------------------------------------
# Relations derived numerically against tabpwa.momenta_to_data (all
...
def _frames_from_angles(chain, ph, th, q, m_node):
    ...
    """
    from tabpwa.su2 import (Identity, Rotation_z, Rotation_y, Boost_z,
                            _mul)
    n = ph.shape[0]
    decays = chain.decays
...
def aligned_euler_from_chain(chain, ph, th, q, m_node, cm_p4,
    ...
    """
    from tabpwa.su2 import _mul, inv as su2_inv, get_euler_angle
