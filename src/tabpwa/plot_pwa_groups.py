...
def config_panels(cfg, data_np, phsp_np):
    ...
    Uses :func:`tabpwa.read_var.vars_from_config` — every variable keeps a
    :meth:`~tabpwa.read_var.ReadVar.read` on the raw kernel event dict, so
    ...
    """
    from tabpwa.read_var import vars_from_config
