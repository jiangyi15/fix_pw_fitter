...
class CUDABackendV2(ComputeBackend):
    ...
    Uses ``CUDAKernelV2`` which allocates all GPU memory (config + scratch)
    in C via ``cuda_create_context_v2``.  Scratch is allocated once and
    reused across ``compute()`` calls — no per-call ``cudaMalloc``/``cudaFree``.

    Supports an optional ``batch_size`` (events per GPU batch, defaults to 0
    ...
    def prepare_phsp_batched(self, phsp_np, n_events):
        """Upload full phsp to GPU once; C code handles internal batching."""
        self._phsp_np = phsp_np
        self._phsp_n = n_events
        self._phsp_scratch = self.kernel.load_data(phsp_np)

    def compute_norm_batched(self, params):
        """Compute norm — v2 C code handles batching internally."""
...
class CUDABackendV2F32(ComputeBackend):
    """v2 CUDA backend — float32 version for 2-4× faster computation.

    Same API as CUDABackendV2 but uses float32 instead of float64.
    Trades numerical precision for speed (gradient precision ~1e-7 vs ~1e-9).
    """
...
class CUDABackendV3(ComputeBackend):
    ...
    Like v2 but uses Catmull-Rom (cubic Hermite) spline interpolation
    for the gamma and FL lookup tables instead of linear interpolation,
    giving C¹ continuity across the table.

    API matches CUDABackendV2 for drop-in replacement.
    """
...
class CUDABackendV3F32(ComputeBackend):
    """v3 CUDA backend — float32 version with Catmull-Rom interpolation.

    Same API as CUDABackendV3 but uses float32 instead of float64.
    Trades numerical precision for speed (gradient precision ~1e-7 vs ~1e-9).
    Uses Catmull-Rom spline interpolation for smoother table lookups.
    """
