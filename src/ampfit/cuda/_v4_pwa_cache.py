...
def _load_lib():
    ...
    ensure("kernels_v4_pwa_cache.cu", "libcuda_kernels_v4_pwa_cache.so")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    lib_path = os.path.join(script_dir, "libcuda_kernels_v4_pwa_cache.so")
...
class CUDAKernelV4PWACache:
    ...
    def __init__(self, config, batch_size=50000, lib_path=None,
        ...
        if n_dev > 0:
            ...
            print(f"v4 PWA cache GPU: {_ffi.string(name_buf).decode()}")
        else:
