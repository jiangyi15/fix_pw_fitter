...
__global__ void reduce_sum_kernel(const float* input, float* output, int n) {
    ...
    sdata[tid] = (idx < n) ? (double)input[idx] : 0.0;
    __syncthreads();
    for (int s = blockDim.x / 2; s > 0; s >>= 1) {
        if (tid < s) sdata[tid] += sdata[tid + s];
        __syncthreads();
    }
    if (tid == 0) atomicAdd(output, (float)sdata[0]);
}
...
__global__ void reduce_sum_complex_kernel(
    ...
    sreal[tid] = (idx < n) ? (double)real_in[idx] : 0.0;
    simag[tid] = (idx < n) ? (double)imag_in[idx] : 0.0;
    __syncthreads();
    for (int s = blockDim.x / 2; s > 0; s >>= 1) {
        if (tid < s) { sreal[tid] += sreal[tid + s]; simag[tid] += simag[tid + s]; }
        __syncthreads();
    }
    if (tid == 0) { atomicAdd(real_out, (float)sreal[0]); atomicAdd(imag_out, (float)simag[0]); }
}
...
__global__ void reduce_sum_features_kernel(
    ...
    for (int i = tid; i < n_events; i += blockDim.x) {
        sum += (double)input[feat + i * n_features];
    }
    sdata[tid] = sum;
    __syncthreads();
    for (int s = blockDim.x / 2; s > 0; s >>= 1) {
    ...
    if (tid == 0) output[feat] = (float)sdata[0];
}
...
__global__ void reduce_sum_complex_features_kernel(
    ...
    for (int i = tid; i < n_events; i += blockDim.x) {
        real_sum += (double)real_in[feat + i * n_features];
        imag_sum += (double)imag_in[feat + i * n_features];
    }
    double* sreal = sdata;
    double* simag = sdata + blockDim.x;
    sreal[tid] = real_sum;
    ...
    if (tid == 0) { real_out[feat] = (float)sreal[0]; imag_out[feat] = (float)simag[0]; }
}
