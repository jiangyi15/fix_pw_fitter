/**
 * OPTIMIZED CUDA kernels (float32) - parallelized inner loops for maximum GPU utilization
 *
 * Key optimizations:
...
__global__ void gram_common_kernel_f32(
    ...
    float* s_ka = s_sh;                 // [0 .. n_angle_k)
    float* s_grp = s_sh + n_angle_k;    // [0 .. 4*ng)
...
//=============================================================================
__global__ void gram_reduce_kernel_f32(
...
// ── Gram matrix kernel launches (f32) ─────────────────────────
void launch_gram_common_f32(
    ...
    gram_common_kernel_f32<<<n_events, BLOCK_SIZE, shmem>>>(
...
void launch_compute_all(
    const ComputeContext* ctx, ComputeData* data,
    const ComputeParams* params, float norm, int use_norm
) {
    launch_compute_g_bw(
...
void* cuda_create_context(
    ...
    const float* ak, int nak, const float* ab, int nab,
    const float* mar, int nmar, const float* mai, int nmai,
    const float* gtr, int ngtr, const float* gti, int ngti,
    float gmin, float gdel, int gbins,
    const float* mg, int nmg,
    const float* ft, int nft, float flmin, float fldel, int fbins,
    ...
    c->angle_k = (float*)_up_dbl(ak, nak);
    c->angle_b = (float*)_up_dbl(ab, nab);
    c->matrix_angle_real = (float*)_up_dbl(mar, nmar);
    c->matrix_angle_imag = (float*)_up_dbl(mai, nmai);
    c->gamma_table_real = (float*)_up_dbl(gtr, ngtr);
    c->gamma_table_imag = (float*)_up_dbl(gti, ngti);
    c->gamma_min = gmin; c->gamma_delta = gdel; c->gamma_table_bins = gbins;
    c->matrix_gamma = (float*)_up_dbl(mg, nmg);
    c->fl_table = (float*)_up_dbl(ft, nft);
...
void* cuda_load_data(void* vctx, const float* mass, const float* mom,
    ...
    d->mass = (float*)_up_dbl(mass, ne * c->n_mass);
    d->momentum = (float*)_up_dbl(mom, ne * c->n_momentum);
    d->angle = (float*)_up_dbl(ang, ne * c->n_angle_total * c->n_angle_comp);
    d->frac = (float*)_up_dbl(frac, ne);
    d->time = (float*)_up_dbl(time, ne);
    d->weight = (float*)_up_dbl(wgt, ne);
    d->bkg = (float*)_up_dbl(bkg, ne);
    d->n_events = ne;
...
void cuda_compute(void* vctx, void* vdh,
    ...
    ComputeParams p;
    p.ck_real = (float*)_up_dbl(ck_r, n_wave);
    p.ck_imag = (float*)_up_dbl(ck_i, n_wave);
    p.m0 = (float*)_up_dbl(m0, n_unique_bw);
    p.g0 = (float*)_up_dbl(g0, n_gamma_rows);
    ...
    // Download results
    cudaMemcpy(Q_out, d->Q_out, ne * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(P_out, d->P_out, ne * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(gck_r, d->grad_ck_real_partial, ne * n_wave * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(gck_i, d->grad_ck_imag_partial, ne * n_wave * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(gm0_out, d->grad_m0_partial, ne * n_unique_bw * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(gg0_out, d->grad_g0_partial, ne * n_gamma_rows * sizeof(float), cudaMemcpyDeviceToHost);

    float buf[6];
    cudaMemcpy(buf, d->grad_Gamma_partial, ne * sizeof(float), cudaMemcpyDeviceToHost);
    gsc_out[0]=0; for(int i=0;i<ne;i++) gsc_out[0]+=buf[i];
    cudaMemcpy(buf, d->grad_DeltaGamma_partial, ne * sizeof(float), cudaMemcpyDeviceToHost);
    gsc_out[1]=0; for(int i=0;i<ne;i++) gsc_out[1]+=buf[i];
    cudaMemcpy(buf, d->grad_DeltaM_partial, ne * sizeof(float), cudaMemcpyDeviceToHost);
    gsc_out[2]=0; for(int i=0;i<ne;i++) gsc_out[2]+=buf[i];
    cudaMemcpy(buf, d->grad_Ap_partial, ne * sizeof(float), cudaMemcpyDeviceToHost);
    gsc_out[3]=0; for(int i=0;i<ne;i++) gsc_out[3]+=buf[i];
    cudaMemcpy(buf, d->grad_poq_rho_partial, ne * sizeof(float), cudaMemcpyDeviceToHost);
    gsc_out[4]=0; for(int i=0;i<ne;i++) gsc_out[4]+=buf[i];
    cudaMemcpy(buf, d->grad_pop_phi_partial, ne * sizeof(float), cudaMemcpyDeviceToHost);
...
void* cuda_create_context_v2_f32(
    ...
    c->angle_k = (float*)_up_dbl(ak, n10); c->angle_b = (float*)_up_dbl(ab, n11);
    c->matrix_angle_real = (float*)_up_dbl(mar, n12); c->matrix_angle_imag = (float*)_up_dbl(mai, n13);
    c->gamma_table_real = (float*)_up_dbl(gtr, n14); c->gamma_table_imag = (float*)_up_dbl(gti, n15);
    c->gamma_min = gmin; c->gamma_delta = gdel; c->gamma_table_bins = gbins;
    c->matrix_gamma = (float*)_up_dbl(mg, n16);
    c->fl_table = (float*)_up_dbl(ft, n17); c->fl_min = flmin; c->fl_delta = fldel; c->fl_table_bins = fbins;
    c->n_wave = nw; c->n_res = nr; c->n_decay = nd;
    c->n_unique_bw = nub; c->n_gamma_rows = ngr;
...
void cuda_free_context_v2_f32(void* vctx) {
    ComputeContext* c = (ComputeContext*)vctx;
...
void* cuda_load_data_v2_f32(void* vctx,
    ...
    h->m = (const float*)_up_dbl(mass, ne * nmass);
    h->mo = (const float*)_up_dbl(mom, ne * nmom);
    h->a = (const float*)_up_dbl(ang, ne * nang * nac);
    h->f = (const float*)_up_dbl(frac, ne);
    h->t = (const float*)_up_dbl(time, ne);
    h->w = (const float*)_up_dbl(wgt, ne);
    h->b = (const float*)_up_dbl(bkg, ne);
    h->ne = ne; h->nm = nmass; h->nmom = nmom; h->nat = nang; h->nac = nac;
    return h;
}
void cuda_free_data_v2_f32(void* vh) {
...
void cuda_gram_matrix_v2_f32(void* vctx, void* vdh,
    const float* m0, const float* g0,
    ...
    // Per-batch scratch: A0/A1 (float32, batch × ng2 complex each)
    ...
    // Per-batch Gram matrix scratch (stored as float32 on GPU)
    ...
    // Zero output on CPU side (output is float64)
    ...
    for (int b = 0; b < nbat; b++) {
        ...
        // Kernel: common_amp → group reduce → A0/A1
        launch_gram_common_f32(c, &d, gpu_m0, A0r, A0i, A1r, A1i);
        CUDA_CHECK(cudaGetLastError());

        // Kernel: reduce A0/A1 → Gram matrix for this batch
        launch_gram_reduce_f32(
            A0r, A0i, A1r, A1i, d.weight, nb, ng2,
            Mpp_r, Mpp_i, Mmm_r, Mmm_i, Mpm_r, Mpm_i);
        CUDA_CHECK(cudaGetLastError());

        // Download batch Gram contribution and accumulate on CPU (as float64)
...
void cuda_compute_v2_f32(void* vctx, void* vdh,
    const float* ck_r,const float* ck_i,
    const float* m0,const float* g0,
    ...
    ComputeParams p;
    p.ck_real = (float*)_up_dbl(ck_r, nw);
    p.ck_imag = (float*)_up_dbl(ck_i, nw);
    p.m0 = (float*)_up_dbl(m0, c->n_m0_params);
    p.g0 = (float*)_up_dbl(g0, c->n_g0_params);
    ...
    *oQ = 0.0; memset(oP, 0, ne * 8);
    ...
    cudaMemset(s.g_bw_real, 0, bs * nu * 4);
    cudaMemset(s.g_bw_imag, 0, bs * nu * 4);
