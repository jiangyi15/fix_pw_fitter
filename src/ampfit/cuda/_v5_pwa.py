...
void cuda_free_data_v5(void*);
void cuda_compute_v5(void*,void*,
    const double*,const double*,const double*,const double*,
    double,int,
    double*,double*,double*,double*,double*,double*,double*);
void cuda_gram_matrix_v5(void*,void*,
...
def _load_lib():
    ...
    ensure("kernels_v5_pwa.cu", "libcuda_kernels_v5_pwa.so")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    lib_path = os.path.join(script_dir, "libcuda_kernels_v5_pwa.so")
...
class DataHandle:
    ...
    def free(self):
        if self.ptr is not None:
            self._lib.cuda_free_data_v5(self.ptr)
            self.ptr = None
            self._keep.clear()
...
class CUDAKernelV5PWA:
    ...
    def __init__(self, config, batch_size=50000, nll_batch=20, lib_path=None):
        ...
        if n_dev > 0:
            ...
            print(f"v5 PWA GPU: {_ffi.string(name_buf).decode()}")
        else:
        ...
        self._ctx = self._lib.cuda_create_context_v5(
    ...
    def load_data(self, data):
        ...
        dh = DataHandle(self._lib.cuda_load_data_v5(
    ...
    def free(self):
        if self._ctx is not None:
            self._lib.cuda_free_context_v5(self._ctx)
            self._ctx = None
        self._ka.clear()
