/**
 * cuda32_v4_pwa_cache — FP32-STORAGE variant of the fixed-m0/g0 full-amplitude
 * cache kernel.  The cached amplitude (event-major common_cache and the
 * column-major common_T that the fpwf forward/reduce kernels stream every
 * call) is stored as float2, halving the steady-state memory traffic
 * (~2x per-call speed vs the FP64 cache, like fpwfitter's FP32 mode).
 * All arithmetic stays in double: only the F matrix itself is fp32
 * (storage precision ~1e-7 per amplitude, well below fpwfitter's FP32
 * pipeline error).  Cache is filled with the FP64 machinery and CAST to
 * float2 at store time.
 *
 * (derived from cuda_v3_ampcache).
 *
...
__global__ void store_common_d2(const double* __restrict__ re,
    ...
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = make_float2((float)re[idx], (float)im[idx]);
}
__global__ void load_common_d2(const float2* __restrict__ in,
                               double* __restrict__ re,
                               double* __restrict__ im, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) { float2 v = in[idx]; re[idx] = (double)v.x; im[idx] = (double)v.y; }
}
...
__global__ void pwa_fpwf_forward_kernel(
    ...
    // FULL-FP32 compute path: the cached amplitude is float2; ck/weight/bkg/
    // norm arrive as double and are narrowed to float at the boundary; all
    // intermediate math is float (only the Q/dnorm scalars accumulate in
    // double to preserve the NLL value).
    int e = blockIdx.x * blockDim.x + threadIdx.x;
    if (e >= ne) return;
    const float norm = (float)norm_d[0];
    const int P = n_proj;
    const int N = n_wave / P;
    ...
    float Ps = 0.0f;
    for (int p = 0; p < P; p++) {
        const float2* Fp = F + (size_t)p * ne + e;   // coalesced across e
        float ar = 0.0f, ai = 0.0f;
        for (int k = 0; k < N; k++) {
            float2 cv = Fp[(size_t)k * neP];
            float cr = (float)ck_real[k], ci = (float)ck_imag[k];
            ar += cr * cv.x - ci * cv.y;
            ai += cr * cv.y + ci * cv.x;
        }
        aR[p] = ar; aI[p] = ai;
    ...
    P_out[e] = (double)Ps;
    float wv = (float)weight[e];
    float bv = (float)bkg[e];
    float dqp;
    if (use_norm == 0) {
        float q = wv * Ps;
        Q_out[e] = (double)q;
        if (qsum) atomicAdd(qsum, (double)q);
        dqp = wv;
    } else {
        float den = Ps + bv * norm;
        float q = -wv * logf(Ps / norm + bv);
        Q_out[e] = (double)q;
        if (qsum) atomicAdd(qsum, (double)q);
        dqp = -wv / den;
        if (dnsum) atomicAdd(dnsum, -(double)Ps * (double)dqp / (double)norm);
    }
    // G row = p*ne + e (FP32 storage), written from registers
    for (int p = 0; p < P; p++) {
        float2* Gp = G + (size_t)p * ne + e;
        Gp[0] = make_float2(dqp * aR[p], -dqp * aI[p]);
    }
}
...
__global__ void fpwf_gradreduce_kernel(
    ...
    // FULL-FP32: F and G are float2; per-thread accumulate in float, block
    // reduce in float, final atomic into the float2 out[k].
    int tid = threadIdx.x;
    int bd = blockDim.x;
    ...
    for (int k = blockIdx.y; k < N; k += gridDim.y) {
        const float2* Fc = F + (long)k * total;
        float ar = 0.0f, ai = 0.0f;
...
__global__ void gram_cache_event_kernel(
    const float2* __restrict__ common,
    const double* __restrict__ weight,
    int nb, int P, int N,
    ...
    int tid = threadIdx.x;
    int bd = blockDim.x;
    const float2* ce = common + (size_t)e * (size_t)P * N;
    ...
    for (int t = tid; t < total; t += bd) {
        ...
        for (int p = 0; p < P; p++) {
            float2 ck = ce[p * N + k];
            float2 cj = ce[p * N + j];
            // conj(ck) * cj  (fp32 amplitude, fp64 accumulate)
            sr += (double)ck.x * cj.x + (double)ck.y * cj.y;
            si += (double)ck.x * cj.y - (double)ck.y * cj.x;
        }
...
typedef struct {
    ...
    float2* dgck;    // [N]  gradient-reduction output (FP32)
    ...
    const float2* graph_ct;  // common_T the graph was captured with
    int graph_valid;
    int graph_attempted;
} ComputeContext;
...
typedef struct {
    ...
    int ne;
    float2* common_cache;    // [ne · n_wave] per-entry a_{p,k}, FP32 storage
    float2* common_T;        // fpwf layout [k][p*ne+e], FP32 storage
    float2* dG;              // [ne*P] gradient vector G (device, FP32)
...
void* cuda_create_context_v4(
    ...
    {
        int N = c->n_wave / c->n_proj;
        if (cudaMalloc(&c->dgck, (size_t)N * sizeof(float2)) != cudaSuccess ||
...
static void launch_common_fill(ComputeContext* c, ComputeData* d,
                             ...
                             float2* dst_cache, int cache_base);
...
int cuda_fill_common_v4_cache(ComputeContext* c, DataHandle2* h,
    ...
    if (h->common_cache == NULL) {
        if (cudaMalloc(&h->common_cache,
                       (size_t)ne * nw * sizeof(float2)) != cudaSuccess)
            return 0;
    }
    if (h->common_T == NULL) {
...
void cuda_compute_v4_cache(void* vctx, void* vdh,
    ...
    if (!h->fpwf_ok) {
        if (cudaMalloc(&h->dG, np_rows * sizeof(float2)) != cudaSuccess ||
    ...
    *oQ = 0;
    ...
    if (c->gstream && c->gexec && c->graph_handle == (void*)h &&
        ...
        cudaMemsetAsync(c->dgck, 0, (size_t)N * sizeof(float2), c->gstream);
        ...
        if (cudaStreamEndCapture(c->gstream, &c->graph) == cudaSuccess &&
            ...
            cudaMemset(c->dgck, 0, (size_t)N * sizeof(float2));
    ...
    {
        float2* zh = (float2*)malloc((size_t)N * sizeof(float2));
        if (zh) {
            cudaMemcpy(zh, c->dgck, (size_t)N * sizeof(float2),
                       cudaMemcpyDeviceToHost);
            for (int j = 0; j < N; j++) {
                ogck_r[j] = (double)zh[j].x;
                ogck_i[j] = (double)zh[j].y;
            }
            free(zh);
        }
    }
