...
//   (max(n_angle_k, 4*ng) × 2 = 672 doubles = 5376 B < 48 KB)
//=============================================================================
__global__ void gram_common_kernel(
    const double* __restrict__ mass,
    ...
    double* s_ka = s_sh;           // [0 .. n_angle_k)    — ka_prod
    double* s_grp = s_sh + n_angle_k;  // [0 .. 4*ng)   — group sums
...
//=============================================================================
// Grid of (ng, ng) blocks × 1 thread each.  Each element sums
//   Mpp[gi,gj] = Σ_event conj(A0[event,gi]) * A0[event,gj]
// over all events in the current batch.
//=============================================================================
...
//=============================================================================
// KERNEL 3: Main forward computation (bw_p, angular factors, amplitudes, prob)
//=============================================================================
// Each block handles one event.
...
void launch_gram_common(
    ...
    // Shared memory: ka_prod (n_angle_k) + group sums (4*ng)
    int shmem = (ctx->n_angle_k + 4 * ng) * sizeof(double);
    gram_common_kernel<<<n_events, BLOCK_SIZE, shmem>>>(
...
void cuda_gram_matrix_v2(void* vctx, void* vdh,
    const double* m0, const double* g0,
    ...
    int nbat = (ne + bs - 1) / bs;
    int nw = c->n_wave, nu = c->n_unique_bw, ng = c->n_gamma_rows;
    int ng2 = nw / 8;  // number of groups (56)
    ...
    // Per-batch scratch: A0/A1 (batch × ng2 complex each)
    ...
    for (int b = 0; b < nbat; b++) {
        ...
        // Zero g_bw scratch (needed fresh per batch)
        ...
        // Kernel 2a: common_amp → group reduce → A0/A1
        launch_gram_common(c, &d, gpu_m0, A0r, A0i, A1r, A1i);
        CUDA_CHECK(cudaGetLastError());
        ...
        // Download batch Gram contribution and accumulate on CPU
