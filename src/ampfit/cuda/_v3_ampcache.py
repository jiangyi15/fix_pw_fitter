"""
CUDA kernel v3 ampcache — sparse Catmull-Rom + cached angular amplitudes.

Copy of the v3 sparse kernel extended with a per-handle cache of the
minimal-set angular amplitudes Amp_s = fa·fl (272 unique values per event
for the current configs), so the per-event per-wave amplitude is

    a_w = ck_w · Amp_slot_of_wave[w](q, angles) / bw_p_w(m)

i.e. the angular factor Amp is pure per-event kinematics and the BW
propagator 1/bw_p is recomputed every iteration from the floating m0/g0.
The angular cache is filled at load_data (fp64 compute, float2 storage —
constant over the fit, so the fp32 rounding is a fixed per-event factor that
does not move the minimum) and reused for every compute() call with no build
check in the hot path.  momentum/angle are uploaded only transiently for the
fill and are NOT kept on the device (mass is kept for the per-iteration BW
recompute).  Unlike cuda_v3_cache this keeps m0/g0 fitted (their gradients
still flow).
"""
...
class CUDAKernelV3AmpCache:
    def __init__(self, config, batch_size=50000, lib_path=None):
        ...
        if n_dev > 0:
            ...
            print(f"v3 ampcache GPU: {_ffi.string(name_buf).decode()}")
        else:
        ...
        # cuda_v3_ampcache: minimal-set angular-cache layout
    ...
    def compute_gram(self, phsp_handle, m0, g0):
        ...
        return (oMpp_r.reshape(ng2, ng2) + 1j * oMpp_i.reshape(ng2, ng2),
                oMmm_r.reshape(ng2, ng2) + 1j * oMmm_i.reshape(ng2, ng2),
                oMpm_r.reshape(ng2, ng2) + 1j * oMpm_i.reshape(ng2, ng2))

    def get_profile(self):
        out_t = _ffi.new("double[10]")
        out_n = _ffi.new("int*")
        self._lib.cuda_get_profile_v3(self._ctx, out_t, out_n)
        names = ["g_bw", "amp_cache_amp", "amp_reduce_time",
                 "grad_ck", "grad_bw_dom", "grad_g0", "grad_scalar"]
        return [(names[i] if i < len(names) else f"k{i}", out_t[i])
                for i in range(min(out_n[0], 10))]
