/**
 * cuda_v5_pwa — projection-sum PWA kernel (derived from cuda_v4_pwa).
 *
 ...
 *     dQ/dP_e = -w_e / (Pb + norm·Bb),  Pb=ΣwP, Bb=Σw·bkg per group
 *
 * The norm=0 (phase-space / norm value) path is unchanged from v4 — linear
 * Σ w·P, so the normalization integral and its gradient are identical.
 * The per-group factor c_g = -1/(Pb + norm·Bb) is applied to the stored
 * conj(A_p) by ``grp_scale_kernel_v5`` once the group sums are known; the
 * BW / m0 / g0 gradient chain downstream is untouched.
 *
...
__global__ void pwa_amp_reduce_kernel(
    ...
    if (tid == 0) {
        ...
        if (use_norm == 0) {
            // per-event linear NLL: Q = Σ w·P, dQ/dA_p = w·conj(A_p)
            double wval = weight[event_idx];
            Q_out[event_idx] = wval * P;
            for (int p = 0; p < n_proj; p++) {
                dQ_dA_r[ab + p] = wval * sA_r[p];
                dQ_dA_i[ab + p] = -wval * sA_i[p];
            }
        } else {
            // cuda_v5: store the raw conj(A_p); the group sums Pb/Bb are
            // reduced by grp_sum_kernel_v5 and grp_scale_kernel_v5 then
            // multiplies each event by w_e·c_g (c_g = -1/(Pb+norm·Bb)).
            for (int p = 0; p < n_proj; p++) {
                dQ_dA_r[ab + p] = sA_r[p];
                dQ_dA_i[ab + p] = -sA_i[p];
            }
...
__global__ void grp_sum_kernel_v5(
    ...
    for (int st = bs / 2; st > 0; st >>= 1) {
        if (tid < st) { sP[tid] += sP[tid + st]; sB[tid] += sB[tid + st]; }
        __syncthreads();
    }
    if (tid == 0) { gP[g] = sP[0]; gB[g] = sB[0]; }
}

//=============================================================================
// KERNEL 3c (cuda_v5): scale stored conj(A_p) by w_e·c_g with
//   c_g = -1 / (Pb_g + norm·Bb_g),  i.e. dQ/dA_p = -w_e·conj(A_p)/(…).
//=============================================================================
__global__ void grp_scale_kernel_v5(
...
//=============================================================================
__global__ void grad_ck_kernel_v5(
    const double* __restrict__ common_r,
    const double* __restrict__ common_i,
...
//=============================================================================
__global__ void grad_bw_dom_kernel_v5(
...
//=============================================================================
__global__ void gram_v5_event_kernel(
    const double* __restrict__ common_r,
    const double* __restrict__ common_i,
    const double* __restrict__ weight,
    int nb, int P, int N,
...
typedef struct {
    ...
    // cuda_v5 log-sum groups: per-group Pb / Bb accumulator scratch [bs]
    double* grpP; double* grpB;
...
typedef struct {
    ...
    // cuda_v5_pwa: angular-cache layout + projection count
...
void launch_compute_all_v5(
    ...
    //── K3: PWA forward (P = Σ_p |A_p|²; per-event or raw dQ/dA) ─────
    {
    ...
    if (use_norm == 1) {
        ...
        double* sums = (double*)malloc((size_t)2 * ng * sizeof(double));
        cudaMemcpy(sums, data->grpP, (size_t)ng * 8, cudaMemcpyDeviceToHost);
        cudaMemcpy(sums + ng, data->grpB, (size_t)ng * 8,
                   cudaMemcpyDeviceToHost);
        double q = 0.0, dn = 0.0;
        for (int g2 = 0; g2 < ng; g2++) {
            double Pb = sums[g2], Bb = sums[ng + g2];
            q += -log(Pb / norm + Bb);
            dn += Pb / (norm * (Pb + Bb * norm));
        }
        *oQ += q;
        *oDn += dn;
        free(sums);

        grp_scale_kernel_v5<<<(ne * P + BLOCK_SIZE - 1) / BLOCK_SIZE,
                              BLOCK_SIZE>>>(
            data->dQ_dA_real, data->dQ_dA_imag,
    ...
    //── K4: ck gradient (projection-reduced partials, length N) ───────
    {
        ...
        grad_ck_kernel_v5<<<ne, BLOCK_SIZE, shmem>>>(
            data->common_amp_factor_real, data->common_amp_factor_imag,
            data->dQ_dA_real, data->dQ_dA_imag,
    ...
    grad_bw_dom_kernel_v5<<<ne, amp_t>>>(
        data->bw_p_real, data->bw_p_imag,
        data->common_amp_factor_real, data->common_amp_factor_imag,
        data->dQ_dA_real, data->dQ_dA_imag,
...
void cuda_free_context_v5(void* vctx);  // forward decl (used by create on error)

void* cuda_create_context_v5(
...
void cuda_free_context_v5(void* vctx) {
...
void* cuda_load_data_v5(void* vctx,
...
void cuda_free_data_v5(void* vh) {
    if (!vh) return;
...
void cuda_compute_v5(void* vctx, void* vdh,
    ...
    *oQ = 0; *oDn = 0; memset(oP, 0, ne * 8);
    ...
    for (int b = 0; b < nbat; b++) {
        ...
        launch_compute_all_v5(c, &d, &p, nv, use_norm, oQ, oDn);
        cudaGetLastError();

        // norm==0: per-event Q_out (linear Σ w·P); norm==1: NLL was
        // accumulated per log-group inside launch_compute_all_v5.
        if (use_norm == 0) {
...
//=============================================================================
void cuda_gram_matrix_v5(void* vctx, void* vdh,
    const double* m0, const double* g0,
