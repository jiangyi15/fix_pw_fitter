...
class CUDALibrary32:
    ...
    def __init__(self, lib_path=None):
        if lib_path is None:
            ...
            lib_path = os.path.join(script_dir, "cuda", "libcuda_kernels_f32.so")
...
class GPUArray:
    ...
    def __init__(self, lib, shape, dtype=np.float32):
        self.lib = lib
    ...
    @classmethod
    def from_ptr(cls, lib, ptr, shape, dtype=np.float32):
...
class GPUDataBuffer32:
    ...
    Usage:
        buf = GPUDataBuffer32(lib, {
            "mass":   ((n, 48), np.float32),
            "q":      ((n, 72), np.float32),
            "angle":  ((n, 72), np.float32),
            "time":   ((n,),   np.float32),
            "frac":   ((n,),   np.float32),
            "weight": ((n,),   np.float32),
            "bkg":    ((n,),   np.float32),
...
class GPUConfig32:
    ...
    def _alloc(self):
        ...
        self.angle_k_gpu = GPUArray(self.lib, (c["angle_k"].size,), np.float32)
        self.angle_k_gpu.set(c["angle_k"].flatten().astype(np.float32))
        self.angle_b_gpu = GPUArray(self.lib, (c["angle_b"].size,), np.float32)
        self.angle_b_gpu.set(c["angle_b"].flatten().astype(np.float32))

        ma = c["matrix_angle"]
        self.matrix_angle_real_gpu = GPUArray(self.lib, ma.shape, np.float32); self.matrix_angle_real_gpu.set(ma.real.astype(np.float32))
        self.matrix_angle_imag_gpu = GPUArray(self.lib, ma.shape, np.float32); self.matrix_angle_imag_gpu.set(ma.imag.astype(np.float32))

        self.matrix_gamma_gpu = GPUArray(self.lib, c["matrix_gamma"].shape, np.float32); self.matrix_gamma_gpu.set(c["matrix_gamma"])

        gt = c["gamma_table"]
        self.gamma_table_real_gpu = GPUArray(self.lib, gt.shape, np.float32); self.gamma_table_real_gpu.set(gt.real.astype(np.float32))
        self.gamma_table_imag_gpu = GPUArray(self.lib, gt.shape, np.float32); self.gamma_table_imag_gpu.set(gt.imag.astype(np.float32))

        self.fl_table_gpu = GPUArray(self.lib, c["fl_table"].shape, np.float32); self.fl_table_gpu.set(c["fl_table"])
...
class GPUDataHolder32:
    ...
    def load(self, data):
        ...
        # ---- input data buffer ----
        self.inputs = GPUDataBuffer32(self.lib, [
            ("mass",   ((ne, self.n_mass), np.float32)),
            ("q",      ((ne, self.n_momentum), np.float32)),
            ("angle",  ((data["angle"].size,), np.float32)),
            ("frac",   ((ne,), np.float32)),
            ("time",   ((ne,), np.float32)),
            ("weight", ((ne,), np.float32)),
            ("bkg",    ((ne,), np.float32)),
        ])
        self.inputs.set("mass", data["mass"])
        self.inputs.set("q", data["q"])
        self.inputs.set("angle", data["angle"].flatten().astype(np.float32))
        self.inputs.set("frac", data["frac"])
        self.inputs.set("time", data["time"])
        self.inputs.set("weight", data["weight"])
        bkg = data.get("bkg", 0.0)
        if np.isscalar(bkg):
            bkg = np.full(ne, bkg, dtype=np.float32)
        self.inputs.set("bkg", bkg)

        # ---- forward output arrays ----
        self.Q_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.P_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.pap_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.pap_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.pam_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.pam_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.gp_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.gp_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.gm_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.gm_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.poq_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.poq_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.bw_p_real_gpu = GPUArray(self.lib, (ne, self.n_wave), np.float32)
        self.bw_p_imag_gpu = GPUArray(self.lib, (ne, self.n_wave), np.float32)
        self.common_amp_factor_real_gpu = GPUArray(self.lib, (ne, self.n_wave), np.float32)
        self.common_amp_factor_imag_gpu = GPUArray(self.lib, (ne, self.n_wave), np.float32)
        self.ap_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.ap_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.am_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.am_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.dQ_dP_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.bw_dom_real_gpu = GPUArray(self.lib, (ne, self.n_unique_bw), np.float32)
        self.bw_dom_imag_gpu = GPUArray(self.lib, (ne, self.n_unique_bw), np.float32)
        self.g_interp_real_gpu = GPUArray(self.lib, (ne, self.n_gamma_rows), np.float32)
        self.g_interp_imag_gpu = GPUArray(self.lib, (ne, self.n_gamma_rows), np.float32)
        self.g_bw_real_gpu = GPUArray(self.lib, (ne, self.n_unique_bw), np.float32)
        self.g_bw_imag_gpu = GPUArray(self.lib, (ne, self.n_unique_bw), np.float32)

        # ---- gradient partial arrays ----
        self.grad_ck_real_partial = GPUArray(self.lib, (ne, self.n_wave), np.float32)
        self.grad_ck_imag_partial = GPUArray(self.lib, (ne, self.n_wave), np.float32)
        self.grad_m0_partial = GPUArray(self.lib, (ne, self.n_unique_bw), np.float32)
        self.grad_g0_partial = GPUArray(self.lib, (ne, self.n_gamma_rows), np.float32)
        self.grad_Gamma_partial = GPUArray(self.lib, (ne,), np.float32)
        self.grad_DeltaGamma_partial = GPUArray(self.lib, (ne,), np.float32)
        self.grad_DeltaM_partial = GPUArray(self.lib, (ne,), np.float32)
        self.grad_Ap_partial = GPUArray(self.lib, (ne,), np.float32)
        self.grad_poq_rho_partial = GPUArray(self.lib, (ne,), np.float32)
        self.grad_pop_phi_partial = GPUArray(self.lib, (ne,), np.float32)

        # ---- reduction output arrays ----
        self.Q_sum_gpu = GPUArray(self.lib, (1,), np.float32)
        self.grad_ck_real_sum_gpu = GPUArray(self.lib, (self.n_wave,), np.float32)
        self.grad_ck_imag_sum_gpu = GPUArray(self.lib, (self.n_wave,), np.float32)
        self.grad_m0_sum_gpu = GPUArray(self.lib, (self.n_unique_bw,), np.float32)
        self.grad_g0_sum_gpu = GPUArray(self.lib, (self.n_gamma_rows,), np.float32)
    ...
    def alloc_intermediates(self, n_events):
        ...
        self.poq_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.poq_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.bw_p_real_gpu = GPUArray(self.lib, (ne, nw), np.float32)
        self.bw_p_imag_gpu = GPUArray(self.lib, (ne, nw), np.float32)
        self.common_amp_factor_real_gpu = GPUArray(self.lib, (ne, nw), np.float32)
        self.common_amp_factor_imag_gpu = GPUArray(self.lib, (ne, nw), np.float32)
        self.ap_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.ap_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.am_real_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.am_imag_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.dQ_dP_gpu = GPUArray(self.lib, (ne,), np.float32)
        self.bw_dom_real_gpu = GPUArray(self.lib, (ne, nb), np.float32)
        self.bw_dom_imag_gpu = GPUArray(self.lib, (ne, nb), np.float32)
        self.g_interp_real_gpu = GPUArray(self.lib, (ne, ng), np.float32)
        self.g_interp_imag_gpu = GPUArray(self.lib, (ne, ng), np.float32)
        self.g_bw_real_gpu = GPUArray(self.lib, (ne, nb), np.float32)
        self.g_bw_imag_gpu = GPUArray(self.lib, (ne, nb), np.float32)
        self.grad_ck_real_partial = GPUArray(self.lib, (ne, nw), np.float32)
        self.grad_ck_imag_partial = GPUArray(self.lib, (ne, nw), np.float32)
        self.grad_m0_partial = GPUArray(self.lib, (ne, nb), np.float32)
        self.grad_g0_partial = GPUArray(self.lib, (ne, ng), np.float32)
        self.grad_Gamma_partial = GPUArray(self.lib, (ne,), np.float32)
        self.grad_DeltaGamma_partial = GPUArray(self.lib, (ne,), np.float32)
        self.grad_DeltaM_partial = GPUArray(self.lib, (ne,), np.float32)
        self.grad_Ap_partial = GPUArray(self.lib, (ne,), np.float32)
        ...
        self.grad_pop_phi_partial = GPUArray(self.lib, (ne,), np.float32)
        self.Q_sum_gpu = GPUArray(self.lib, (1,), np.float32)
        self.grad_ck_real_sum_gpu = GPUArray(self.lib, (nw,), np.float32)
        self.grad_ck_imag_sum_gpu = GPUArray(self.lib, (nw,), np.float32)
        self.grad_m0_sum_gpu = GPUArray(self.lib, (nb,), np.float32)
        self.grad_g0_sum_gpu = GPUArray(self.lib, (ng,), np.float32)
        self.data_loaded = True
...
class GPUData32(GPUDataHolder32):
    ...
    def _alloc_config_gpu(self):
        ...
        self.fl_order_gpu = GPUArray(self.lib, (len(c["fl_order"]),), np.int32); self.fl_order_gpu.set(c["fl_order"])
        self.angle_index_gpu = GPUArray(self.lib, c["angle_index"].shape, np.int32); self.angle_index_gpu.set(c["angle_index"])
        self.angle_k_gpu = GPUArray(self.lib, (c["angle_k"].size,), np.float32); self.angle_k_gpu.set(c["angle_k"].flatten().astype(np.float32))
        self.angle_b_gpu = GPUArray(self.lib, (c["angle_b"].size,), np.float32); self.angle_b_gpu.set(c["angle_b"].flatten().astype(np.float32))
        ma = c["matrix_angle"]
        self.matrix_angle_real_gpu = GPUArray(self.lib, ma.shape, np.float32); self.matrix_angle_real_gpu.set(ma.real.astype(np.float32))
        self.matrix_angle_imag_gpu = GPUArray(self.lib, ma.shape, np.float32); self.matrix_angle_imag_gpu.set(ma.imag.astype(np.float32))
        self.matrix_gamma_gpu = GPUArray(self.lib, c["matrix_gamma"].shape, np.float32); self.matrix_gamma_gpu.set(c["matrix_gamma"])
...
class CUDAKernel32:
    ...
    def __init__(self, config):
        ...
        try:
            self.lib = CUDALibrary32()
            self.cuda_available = True
            self.gpu_config = GPUConfig32(config, self.lib)
    ...
    def load_data(self, data):
        ...
        gc = self.gpu_config
        holder = GPUDataHolder32(self.lib, gc.n_wave, gc.n_unique_bw, gc.n_gamma_rows)
        holder.load(data)
        return holder
    ...
    def _compute_cuda(self, data_holder, params, norm):
        ...
        ck_real = np.real(params["ck"]).astype(np.float32)
        ck_imag = np.imag(params["ck"]).astype(np.float32)

        ck_real_gpu = GPUArray(lib, (len(params["ck"]),), np.float32); ck_real_gpu.set(ck_real)
        ck_imag_gpu = GPUArray(lib, (len(params["ck"]),), np.float32); ck_imag_gpu.set(ck_imag)
        m0_gpu = GPUArray(lib, (len(params["m0"]),), np.float32); m0_gpu.set(params["m0"])
        g0_gpu = GPUArray(lib, (len(params["g0"]),), np.float32); g0_gpu.set(params["g0"])
...
if __name__ == "__main__":
    ...
    if not os.path.exists("libcuda_kernels_f32.so"):
