"""
CUDA kernel v3 split — Catmull-Rom interpolation backend with split kernels.

Split version of v3: forward (compute_main) split into bw_amp + amp_reduce_time.
Backward (gradient) split into grad_ck + grad_bw_dom + grad_g0 + grad_scalar.
Reduces register pressure from 56→48 (forward) and 80→48 (gradient).
...
void cuda_get_profile_v3(void*,double*,int*);
void cuda_enable_profile_v3(void*,int);
""")


def _load_lib():
    """Load the v3 split shared library, auto-building if source changed."""
    from ampfit.cuda.build import ensure
    ensure("kernels_v3_split.cu", "libcuda_kernels_v3_split.so")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    lib_path = os.path.join(script_dir, "cuda", "libcuda_kernels_v3_split.so")
    if not os.path.exists(lib_path):
        raise RuntimeError(
            f"v3 split CUDA library not found at {lib_path}. "
            "Build with: nvcc -shared -fPIC -o <path> kernels_v3_split.cu -lcudart"
        )
...
class CUDAKernelV3Split:
    """High-level v3 split CUDA kernel backend (Catmull-Rom interpolation).

    Split-kernel variant: forward is bw_amp→amp_reduce_time,
    backward is grad_ck→grad_bw_dom→grad_g0→grad_scalar.
    Uses same extern \"C\" API as v3 (identical CFFI bindings).

    Usage::

        kernel = CUDAKernelV3Split(config)
        data = kernel.load_data(data_dict)
        Q, grads, P = kernel.compute(params, data)

    Compatible return format with v3 CUDAKernelV3 for drop-in replacement.
    """
    ...
    def __init__(self, config, batch_size=50000, lib_path=None):
        ...
        if n_dev > 0:
            ...
            print(f"✓ v3 split GPU (Catmull-Rom): {_ffi.string(name_buf).decode()}")
        else:
    ...
    def get_profile(self):
        """Return per-kernel timing profile (ms per segment) after compute().

        Returns list of (name, ms) tuples.  Resets accumulated profile.
        """
        out_t = _ffi.new("double[10]")
        out_n = _ffi.new("int*")
        self._lib.cuda_get_profile_v3(self._ctx, out_t, out_n)
        names = ["g_bw", "compute_fa", "bw_amp", "amp_reduce_time",
                 "grad_ck", "grad_bw_dom", "grad_g0", "grad_scalar"]
        n = out_n[0]
        return [(names[i] if i < len(names) else f"k{i}",
                 out_t[i]) for i in range(min(n, 10))]

    def enable_profile(self, enable=True):
        """Enable/disable per-kernel profiling.

        Profiling adds ~5ms sync overhead per compute() call.
        Disabled by default.  Enable before compute(), then call
        get_profile() after compute() to retrieve timings.
        """
        self._lib.cuda_enable_profile_v3(self._ctx, 1 if enable else 0)

    # -- gram matrix (phsp pre-integration) -----------------------------------

    def compute_gram(self, phsp_handle, m0, g0):
        """Compute reduced Gram matrices from phsp data on GPU (v3 split).
