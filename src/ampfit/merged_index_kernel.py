"""
Merged-index BW kernel that eliminates scatter step.

Key optimization: Instead of computing BW at unique positions and scattering,
...
Merged-index flow:
  ...
  bw_dom_all = m0_at_bw² - m0_m_at_bw² - 1j*m0_at_bw*g_bw_at_bw  # (N, 896)
"""
import numpy as np


class MergedIndexKernel:
    """Gradient computation with merged-index BW optimization"""
    ...
    def __init__(self, config):
        ...
        # MERGED INDEX: Precompute merged indices for direct BW computation
        self.bw_m0_index = self.m0_index[self.bw_order]  # (896,) m0 index at each wave×res position
        self.bw_mass_index = self.mass_index[self.bw_order]  # (896,) mass index at each wave×res position
        
        # Scatter matrices for merged-index backward pass
        # bw_m0_scatter: maps from (896,) at wave×res positions to (n_m0_unique,)
        self.bw_m0_scatter = np.zeros((n_wave_n_res, n_m0_unique), dtype=np.float64)
        for i, m_idx in enumerate(self.bw_m0_index):
            self.bw_m0_scatter[i, m_idx] = 1.0
        
        # bw_gamma_scatter: maps from (896,) at wave×res positions to (n_unique_bw,) unique BW positions
        self.bw_gamma_scatter = np.zeros((n_wave_n_res, n_unique_bw), dtype=np.float64)
        for i, bw_idx in enumerate(self.bw_order):
            self.bw_gamma_scatter[i, bw_idx] = 1.0
    ...
    def _compute(self, params, data, norm=None):
        """Compute forward and gradients with merged-index BW"""
        ...
        # Gamma interpolation (SAME as original)
        ...
        # MERGED INDEX: direct m0 and mass for each position
        m0_at_bw = np.take(m0, self.bw_m0_index)  # (896,)
        m0_m_at_bw = np.take(mass, self.bw_mass_index, axis=-1)  # (N, 896)
        
        # BW directly at all 896 positions
        bw_dom_all = m0_at_bw**2 - m0_m_at_bw**2 - 1j * m0_at_bw * g_bw_at_bw  # (N, 896)
        n_events = bw_dom_all.shape[0]
        bw_dom_all_reshaped = bw_dom_all.reshape(n_events, self.n_wave, self.n_res)
        bw_p = np.prod(bw_dom_all_reshaped, axis=-1)  # (N, n_wave)
        ...
        # dQ/dbw_dom_all is already at (N, 896) from dQ_dbw_p * product chain
        dQ_dbw_dom_all = np.stack([
            dQ_dbw_p * bw_dom_all_reshaped[:, :, 1],
            dQ_dbw_p * bw_dom_all_reshaped[:, :, 0],
        ], axis=-1).reshape(n_events, -1)  # (N, 896)

        # ==================== MERGED-INDEX GRADIENTS ====================
        
        # ── Gradient for m0 ──
        # bw_dom = m0² - m² - 1j*m0*g_bw at each BW position
        # dbw_dom/dm0 = 2*m0 - 1j*g_bw at each BW position
        dbw_dom_dm0_at_bw = 2 * m0_at_bw - 1j * g_bw_at_bw  # (N, 896)
        dm0_raw = 2 * np.real(dQ_dbw_dom_all * dbw_dom_dm0_at_bw)  # (N, 896)
        dm0_sum = np.sum(dm0_raw, axis=0)  # (896,) — at wave×res positions
        # Scatter-sum by m0_index[bw_order] → one grad per unique m0
        grad_m0 = dm0_sum @ self.bw_m0_scatter  # (n_m0_unique,)
        
        # ── Gradient for g0 ──
        # dbw_dom/dg_bw = -1j*m0 at each BW position
        dQ_dg_bw_at_bw = dQ_dbw_dom_all * (-1j * m0_at_bw)  # (N, 896)
        # Scatter-sum back to unique BW positions via bw_order
        dQ_dg_bw_unique = dQ_dg_bw_at_bw @ self.bw_gamma_scatter  # (N, n_unique_bw)
        # Then same as original: chain through gamma interpolation and g0_index
        dQ_dg = dQ_dg_bw_unique @ self.matrix_gamma.T  # (N, n_gamma_rows)
        dg0_raw = 2 * np.real(dQ_dg * g_interp)  # (N, n_gamma_rows)
