...
def _load_lib():
    """Load the v2 float32 shared library."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    lib_path = os.path.join(script_dir, "cuda", "libcuda_kernels_mixed.so")
...
class CUDAKernelMixed:
    """High-level v2 float32 CUDA kernel backend.

    Usage::

        kernel = CUDAKernelMixed(config)
    ...
    def __init__(self, config, batch_size=50000, lib_path=None):
        ...
        if n_dev > 0:
            ...
            print(f"✓ mixed v2+v3 f64 GPU: {_ffi.string(name_buf).decode()}")
        else:
    ...
    def compute(self, params, data_handle, norm=None):
        ...
        m0 = np.ascontiguousarray(params["m0"], np.float32)
        g0 = np.ascontiguousarray(params["g0"], np.float32)
