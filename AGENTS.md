...
| **v4 PWA cache** | `cuda_v4_pwa_cache` | Same pure-PWA model, **standalone full-amplitude cache for fixed m0/g0** (fpwfitter-style): per-handle `common[e, p·N+k]` (angular × BW at the fixed m0/g0 bound at kernel construction) built during load_data, then every iteration only contracts `ck`. Verified bit-equal to `cuda_v4_pwa` at fixed m0/g0 (Q/P/ck-grad; m0/g0 grads zero). **Strictly fixed-m0/g0**: no auto refill and no auto switch — float masses/widths ⇒ use `cuda_v4_pwa`. |
